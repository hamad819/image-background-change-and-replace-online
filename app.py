...
if use_local_image:
    ...
    if subject_file != None and background_file != None:
        subject_name = subject_file.name
        background_name = background_file.name
        
        subject_img = Image.open(subject_file)
        subject_img_path = os.path.join('original', subject_name)
        subject_img.save(subject_img_path, format='jpeg')

        background_img = Image.open(background_file)
        background_img_path = os.path.join('original', background_name)
        background_img.save(background_img_path, format='jpeg')
        ...
        name = subject_file.name[::-1].split('.')[-1][::-1]
        img_name = os.path.join("masked",name) + ".png"
        subject.save(img_name)


        cols[0].image(subject, caption="Subject Image without Background. Use Slider to Control Background Removal", use_column_width=True)

        
        background_img = background_img.resize(subject.size)
        background_img.paste(subject, (0,0), subject    )
